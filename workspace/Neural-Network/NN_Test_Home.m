...
input_data_matrix = str2double(input_data_matrix);
input_data_matrix(257:328,:) = [];
input_data_matrix(63:154,:) = [];%
input_data_matrix(62:69,:) = [];%8
[emailNum,~] = size(input_data_matrix);
trainNum = emailNum;
%{
for i = 1 : 427
    for j = 1 : 54
...
input_test_data = input_data_matrix(1:end,1:species_totalNum);
input_test_matrix = input_test_data';
output_test_matrix = input_data_matrix(1:end,end)';
%%%Data Normailization
...
layerNum = 16;
%nNet = newff(input_train_matrix,output_train_matrix,[64,64,64,64],{'tansig','purelin'},'traingdx');
nNet = newff(input_train_matrix,output_train_matrix,[64,16]);
